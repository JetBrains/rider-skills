...
|------|---------|----------|
| `viewport_camera` | Position the editor camera precisely | `set` location/rotation to match a Sequencer key for visual verification; `focus_on_actor` to frame a CineCamera |
| `take_screenshot` | Capture composition frames | After positioning via `viewport_camera`, capture `kind="viewport"` to compare against reference storyboard frames |
| `ue_execute_python` | Read/set CineCameraComponent properties | Adjust `CurrentFocalLength`, `CurrentAperture`, `ManualFocusDistance` on the active CineCamera at runtime |
| `ue_status` | Check PIE state before camera moves | Confirm the editor is idle (not in PIE) before calling `viewport_camera set` — camera moves are editor-only |
