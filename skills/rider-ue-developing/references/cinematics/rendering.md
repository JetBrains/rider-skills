...
|------|---------|----------|
| `ue_execute_python` | Submit MRQ jobs programmatically | Create a render queue, configure AA settings and CVars, submit and monitor the job — no manual UI needed |
| `ue_play` | Warm-up the editor before rendering | Run PIE briefly to pre-warm texture streaming before submitting a final MRQ job (`mode="viewport"`) |
| `ue_get_logs` | Monitor render progress | `category="LogMoviePipeline"`, `minVerbosity="Log"` — watch per-frame render times and catch pipeline errors |
| `ue_status` | Confirm editor health before long render | Check PIE state and error log before kicking off a multi-hour offline render |
