...
|------|---------|----------|
| `ue_execute_python` | Run gotcha-aware creation scripts | Execute the safe patterns described above (compile → save; SubobjectDataSubsystem flow) directly in the running editor |
| `ue_status` | Check editor state and recent logs | Before any bulk Blueprint operation, verify no PIE is running and inspect `LogBlueprint` for prior compile errors |
| `ue_get_logs` | Surface silent failure signals | `category="LogBlueprint"`, `category="LogGarbageCollector"`, `minVerbosity="Warning"` — expose errors that Python returns no exception for |
| `search_assets` | Verify asset paths before creation | Confirm a `/Game/Path/BP_Name` exists or doesn't before calling `create_asset` (avoids "returns None" gotcha #1) |
| `get_file_problems` | Check C++ parent class before referencing | Ensure the parent C++ class has no compile errors before `factory.set_editor_property("parent_class", ...)` |
