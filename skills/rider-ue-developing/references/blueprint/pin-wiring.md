...
|------|---------|----------|
| `ue_execute_python` | Discover actual pin names on a live node | Call `ab.get_blueprint_graph_nodes(bp_path, "EventGraph")` — each node entry includes its pins with internal names |
| `ue_status` | Check editor idle before graph inspection | Graph queries fail silently during PIE; confirm state is `Idle` first |
| `ue_get_logs` | Surface silent wiring failures | `category="LogBlueprint"`, `minVerbosity="Warning"` — wrong pin names produce no exception but log a warning on compile |
