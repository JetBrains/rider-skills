...
|------|---------|----------|
| `ue_play` | Start PIE for load testing | Spawn large NPC counts, trigger streaming level loads, or simulate player join-in-progress to stress-test the architecture |
| `ue_execute_python` | Measure memory and actor counts | Query `UWorld.get_world().get_actor_count()` or `get_all_actors_of_class` during PIE to validate LOD and culling thresholds |
| `ue_get_logs` | Watch World Partition and streaming logs | `category="LogWorldPartition"`, `category="LogStreaming"`, `minVerbosity="Warning"` — catch streaming failures at scale |
| `build_solution_start` / `build_solution_state` | Compile after splitting a monolithic module | Validate that the new module boundary compiles cleanly before checking in |
