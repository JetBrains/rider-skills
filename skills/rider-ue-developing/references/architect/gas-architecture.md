...
|------|---------|----------|
| `ue_execute_python` | Inspect live attribute values and granted abilities | Query `UAbilitySystemComponent` state at runtime: active tags, attribute base/current values, granted ability handles |
| `ue_play` | Run gameplay scenarios for GAS validation | Start PIE to trigger ability activations and verify cost deduction, cooldown application, cue spawning |
| `ue_get_logs` | Stream GAS diagnostic logs | `category="LogAbilitySystem"`, `minVerbosity="Verbose"` — trace activation, tag requirements, and effect application |
| `search_symbol` | Find all `UGameplayAbility` subclasses | Locate every ability in the codebase to audit activation groups and cooldown tag usage |
| `analyze_calls` | Map effect application flow | `OUTGOING_CALLS` on `ApplyGameplayEffectToSelf` to understand what triggers attribute changes |
