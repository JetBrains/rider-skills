...
|------|---------|----------|
| `search_symbol` | Audit how much of a pattern exists already | Before choosing GAS: find all `UAbilitySystemComponent` users; before choosing Mass: find all AI actor counts |
| `analyze_calls` | Measure coupling before choosing decoupling pattern | `INCOMING_CALLS` on a type to see how many callers would need changing if you introduce an interface layer |
| `get_project_problems` | Check baseline health before a structural decision | Confirm zero errors before deciding whether to split a module or introduce a new system |
