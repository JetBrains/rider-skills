...
|------|---------|----------|
| `ue_play` | Start PIE | Trigger AI movement to observe navmesh pathfinding live |
| `ue_get_logs` | Stream nav system logs | `category="LogNavigation"`, `category="LogPathFollowing"`, `minVerbosity="Warning"` — catch stuck paths and partial path errors |
| `ue_execute_python` | Query nav agent state at runtime | Check `character_movement.is_moving_on_ground()`, request a test path via `UNavigationSystemV1` |
| `viewport_camera` | Frame AI pawn for visual debugging | `focus_on_actor` on the AI pawn, then check navmesh overlay (P key) |
