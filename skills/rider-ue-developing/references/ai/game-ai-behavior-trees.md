...
|------|---------|----------|
| `ue_play` | Start / stop PIE | Run the AI scenario to observe tree execution |
| `ue_get_logs` | Stream BT logs | `category="LogBehaviorTree"`, `minVerbosity="Verbose"` — trace active node path |
| `ue_execute_python` | Read Blackboard at runtime | Inspect `BB_CurrentEnemy`, `BB_LastKnownLocation` values live |
| `search_assets` | Find BehaviorTree assets | Locate the production BT for `get_asset_properties` review |
