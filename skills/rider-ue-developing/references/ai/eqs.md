...
|------|---------|----------|
| `ue_play` | Start / stop PIE | Run the AI scenario to trigger EQS queries in flight |
| `ue_get_logs` | Stream EQS diagnostic output | `category="LogEQS"`, `minVerbosity="Warning"` — catch zero-item results and filter over-rejection |
| `ue_execute_python` | Inspect last EQS result on a pawn | `ai_controller.get_component_by_class(unreal.EnvQueryManager)` — check run state |
| `search_assets` | Find UEnvQuery assets | Locate EQS query assets by name for `get_asset_properties` review |
| `get_asset_properties` | Read EQS query CDO defaults | Inspect generator type, test thresholds, context without opening the editor |
