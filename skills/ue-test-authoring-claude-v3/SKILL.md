---
name: ue-test-authoring-claude-v3
...
For a source-only change to one or two files: after the file is fixed, run one focused diagnostic check and one final `reformat_file`. Their successful results are final evidence. Do not re-read, re-lint, or re-run diagnostics after a successful reformat solely to confirm it; only run another check after making a further edit or when the first result reported an actionable issue.
...
7. When a protected UE hook is deliberately the unit under test — an attribute-set clamp, for example — prefer a source-only Automation test and widen access narrowly with `#define protected public` around only the tested header include. Do not escalate to maps, PIE, Functional Tests, Blueprint tests, or Gauntlet unless the behavior truly needs them.
...
9. For `UAttributeSet` tests, check whether the accessors under test route through `GetOwningAbilitySystemComponent()`. If the test constructs the attribute set directly, seed backing values with `InitXxx` or raw attribute data before invoking the hook under test; call `SetXxx` accessors only if the test also creates a minimal owning actor plus `UAbilitySystemComponent`, initializes actor info, and registers the attribute set with that component.
10. For an `UAttributeSet` clamp, call the actual `PreAttributeBaseChange` and `PreAttributeChange` hooks from source (widen access narrowly if required). Seed the relevant maximum attribute, then assert a recognizable high outlier such as `150.0f` or `175.0f` clamps to that maximum, and assert a negative input clamps to `0.0f`. Do not substitute a helper or infer the hook from a call site.
