...
All tools: `mcp__<prefix>__<tool>`. **Never call without loading the schema first — use Codex `tool_search` with a query like `select:mcp__<prefix>__<tool>` before any call.**
...
**Detect mode:** After `tool_search`, if only `execute_tool` is returned → CLI mode. Call `tool_search` with `select:mcp__<prefix>__execute_tool` to get the live CLI schema.
