...
1. Write or edit file using `apply_patch`
2. `get_file_problems` → errors? → edit to fix → back to 2
3. No errors → move on

## Full quality pass

1. Write all files using `apply_patch`
...
Use `search_symbol` to check. Use `rg --files`, `find`, or `list_directory_tree` to check file layout.
