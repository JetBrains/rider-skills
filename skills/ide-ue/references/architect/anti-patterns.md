...
|------|---------|----------|
| `ue_play` | Reproduce the anti-pattern live | Start PIE to observe spaghetti Event Graph tick behavior or AI performance in action |
| `ue_get_logs` | Surface warnings related to the anti-pattern | `category="LogNet"`, `category="LogBehaviorTree"`, `minVerbosity="Warning"` — many anti-patterns produce log warnings |
| `search_symbol` | Find all usages of a problematic pattern | Locate every `GetAllActorsOfClass` call in Tick or every bare `AActor` raw pointer before fixing |
| `analyze_calls` | Trace call paths for architectural violations | `INCOMING_CALLS` on a function to detect which systems are illegally calling across module boundaries |
