...
|------|---------|----------|
| `search_assets` | Find assets by name or base class | Verify the asset registry can find a `UPrimaryDataAsset` subclass before registering it with Asset Manager |
| `get_asset_properties` | Read CDO defaults from a data asset | Inspect `PrimaryDataAsset` fields to confirm values loaded from a DataTable or DataAsset JSON are correct |
| `ue_execute_python` | Audit async load handles at runtime | Call `UAssetManager.get_asset_manager().get_primary_asset_type_info_list()` to inspect configured primary asset types |
| `ue_get_logs` | Surface asset streaming issues | `category="LogStreaming"`, `category="LogAssetManager"`, `minVerbosity="Warning"` — catch broken soft references and load failures |
