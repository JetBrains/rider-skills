...
|------|---------|----------|
| `simulate_input` | Inject input events for testing | `mode="enhanced"` with `enhancedAssetPath` to drive an `IA_Move` or `IA_Look` input action without physical hardware |
| `ue_play` | Start PIE for input / camera validation | Trigger Enhanced Input mapping context switches and verify no ghost inputs or context leaks |
| `ue_get_logs` | Stream input and camera logs | `category="LogEnhancedInput"`, `category="LogCamera"`, `minVerbosity="Verbose"` — trace context activation/deactivation |
| `search_assets` | Find Input Action and Input Mapping Context assets | `search_assets(query="IA_", baseClass="InputAction")` to list all actions before auditing context assignments |
