...
|------|---------|----------|
| `ue_execute_python` | Create or configure a ViewModel/widget programmatically | Instantiate a `ULyraHUDLayout`, populate a ViewModel field, and push it to the UI layer without clicking through the editor |
| `ue_play` | Validate the full UI stack at runtime | Start PIE and exercise widget activation, focus transitions, and CommonUI back-handler chains |
| `take_screenshot` | Capture UI state for review | `take_screenshot(kind="viewport")` at a specific game state to document layout, Z-order, and slot fill |
| `search_assets` | Find all widget classes or layouts | `search_assets(query="HUDLayout", baseClass="CommonActivatableWidget")` to inventory the UI hierarchy |
