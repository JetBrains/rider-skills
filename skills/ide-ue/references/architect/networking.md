...
|------|---------|----------|
| `ue_play` | Start multiplayer PIE | `mode="standalone"`, `players=2` — spin up two clients to test RPC delivery and replication order |
| `ue_get_logs` | Stream replication logs | `category="LogNet"`, `category="LogRepTraffic"`, `minVerbosity="Verbose"` — trace property updates and RPC calls |
| `ue_execute_python` | Read actor replication state at runtime | Dump `GetNetMode()`, `GetLocalRole()`, `GetRemoteRole()` on key actors from the editor Python API |
| `search_symbol` | Find all `UPROPERTY(Replicated)` declarations | Locate every replicated property in a subsystem to audit replication conditions |
| `xdebug_set_breakpoint` | Break on an OnRep callback | Set a conditional breakpoint in `OnRep_Health` to catch unexpected replication order issues |
