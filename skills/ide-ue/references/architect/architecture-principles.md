...
|------|---------|----------|
| `search_symbol` | Audit adherence to architecture principles | Grep for `GetAllActorsOfClass`, raw UObject pointers, or direct subsystem references in the wrong module |
| `analyze_calls` | Validate separation of concerns | `OUTGOING_CALLS` on a game module type to confirm it doesn't reach into editor-only or plugin-only code |
| `build_solution_start` / `build_solution_state` | Verify the module structure compiles cleanly | After restructuring modules or adding dependencies, confirm no circular build errors |
| `get_project_problems` | Check solution-wide health before a large refactor | Establish a clean baseline — confirm zero pre-existing errors before applying architectural changes |
