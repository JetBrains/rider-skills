...
|------|---------|----------|
| `ue_play` | Start PIE to observe AI behavior | Trigger NPC scenarios and watch Behavior Tree / State Tree execution live |
| `ue_get_logs` | Stream AI logs | `category="LogBehaviorTree"`, `category="LogStateTree"`, `minVerbosity="Verbose"` — trace active node and service execution |
| `ue_execute_python` | Query Significance Manager state at runtime | Dump significance levels and tick rates for all AI actors to validate LOD behavior |
| `search_symbol` | Find all `UBrainComponent` / `UAIController` subclasses | Audit the AI controller hierarchy before adding a new perception or decision system |
