...
|------|---------|----------|
| `execute_run_configuration` | Run a test configuration | Execute an Automation Test run configuration via Rider; collect `output` for failure analysis |
| `get_file_problems` | IDE diagnostics for a test file | Catch compile errors in test classes before running the test suite |
| `build_solution_start` / `build_solution_state` | Compile before running tests | Ensure test binaries are up to date; `buildIsSuccess == true` before firing tests |
| `ue_play` | Start PIE for latent / functional tests | Functional tests require PIE; use `mode="standalone"` for isolated test runs |
| `ue_get_logs` | Stream test output | `category="LogAutomationController"`, `category="LogAutomation"`, `minVerbosity="Log"` — surface PASSED / FAILED lines |
| `xdebug_set_breakpoint` | Break inside a failing test | Set a conditional breakpoint in the test assertion path for step-through debugging |
