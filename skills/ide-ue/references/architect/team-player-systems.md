...
|------|---------|----------|
| `ue_play` | Test team scenarios in multiplayer PIE | Start 2+ player PIE with `netMode="listen"` to exercise team assignment, spawn selection, and score replication |
| `ue_get_logs` | Watch team subsystem events | `category="LogLyraTeam"`, `minVerbosity="Log"` — confirm assignment messages, friendly-fire checks, and color updates all fire |
| `ue_execute_python` | Query team state at runtime | `get_all_actors_of_class(LyraTeamInfoBase)` to inspect team registrations mid-PIE |
| `search_symbol` | Find all team assignment call sites | Locate every place `AssignToTeam` or `GetTeam` is called to audit join-in-progress coverage |
