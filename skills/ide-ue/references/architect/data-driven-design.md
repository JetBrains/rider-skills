...
|------|---------|----------|
| `get_asset_properties` | Read DataAsset / DataTable CDO values | Inspect attribute base values, item fragment lists, or config thresholds without opening the editor |
| `search_assets` | Find all assets of a given primary type | `search_assets(baseClass="PrimaryDataAsset")` to enumerate all data-driven configuration assets |
| `find_default_value_overrides` | Audit which Blueprints override a UPROPERTY | Find every BP that overrides a fragment class or attribute set value before a bulk data migration |
| `ue_execute_python` | Batch-update DataAsset defaults | Apply a balance pass across all `DA_Item_*` assets via `get_default_object` + `set_editor_property` |
| `ue_get_logs` | Catch asset load failures | `category="LogAssetManager"`, `minVerbosity="Warning"` — surface broken `TSoftObjectPtr` or missing Primary Asset registrations |
