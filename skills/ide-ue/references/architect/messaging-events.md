...
|------|---------|----------|
| `ue_execute_python` | Inspect active message listeners at runtime | Query `UGameplayMessageSubsystem` listener registrations from a live PIE session to detect missing subscriptions |
| `ue_play` | Fire events and observe cross-system reactions | Start PIE, kill an NPC, and verify that achievement and HUD systems both receive the kill message |
| `ue_get_logs` | Trace message dispatch | `category="LogGameplayMessageRouter"`, `minVerbosity="Log"` — confirm each message reaches all expected listeners |
| `search_symbol` | Find all `RegisterListener` calls | Locate every place a system subscribes to a specific `FGameplayTag` message type |
| `analyze_calls` | Map message dispatch flow | `OUTGOING_CALLS` on `BroadcastMessage` to see which tags are published from a given system |
