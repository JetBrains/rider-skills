...
|------|---------|----------|
| `ue_play` | Run the player-facing scene | Start PIE to experience the verb in context — the fastest way to validate that a mechanic communicates intent |
| `take_screenshot` | Capture a scene moment | Document how the environment communicates a rule or consequence; compare iterations |
| `ue_execute_python` | Script a gameplay scenario | Spawn items, trigger events, or set game state to force a specific scene for rapid design iteration |
