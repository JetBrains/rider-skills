...
|------|---------|----------|
| `ue_execute_python` | Check experience load state at runtime | Query `ULyraExperienceManagerComponent` state or `UGameFeatureSubsystem` plugin status from a PIE session |
| `ue_play` | Trigger experience loading | Start PIE to exercise the async load → OnExperienceLoaded pipeline and verify all `CallOrRegister` callbacks fire |
| `ue_get_logs` | Watch game feature and experience logs | `category="LogGameFeatures"`, `category="LogLyraExperience"`, `minVerbosity="Log"` — trace activation and action registration |
| `search_assets` | Find GameFeature plugin data assets | Locate the `ULyraExperienceDefinition` and `ULyraExperienceActionSet` assets that drive an experience |
