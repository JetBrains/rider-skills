...
|------|---------|----------|
| `search_symbol` | Find all components implementing an interface | Locate every `IGameFrameworkInitStateInterface` implementor to understand the init-state graph before adding a new component |
| `analyze_calls` | Trace init-state dependency chain | `OUTGOING_CALLS` on `CheckDefaultInitialization` to see which features a component depends on reaching `GameplayReady` |
| `get_symbol_info` | Inspect a component's signature and base class | Verify base class and available overrides before subclassing `UPawnComponent` |
| `build_solution_start` / `build_solution_state` | Compile after adding new component class | Validate generated headers and UCLASS export; `buildIsSuccess` required before registering the feature with ComponentManager |
| `xdebug_set_breakpoint` | Break on init-state transition | Conditional breakpoint in `HandleChangeInitState` on the target `DesiredState` to debug ordering issues |
