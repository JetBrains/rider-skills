...
|------|---------|----------|
| `ue_execute_python` | Run Blueprint creation/modification scripts | Pass the Python snippets above directly; use `script` for single operations or `scripts` array for compile → create → save pipelines |
| `ue_health` | Verify editor is connected before scripting | Call once before any `ue_execute_python` batch; fails fast if the editor is not running |
| `ue_status` | Check PIE state and recent logs | Confirm no active PIE session (Blueprint edits fail during PIE); inspect `LogBlueprint` entries for compile errors |
| `search_assets` | Find a Blueprint asset by name | `search_assets(query="BP_MyActor")` to confirm the asset path before calling `create_asset` or `load_asset` |
```
