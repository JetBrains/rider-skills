...
|------|---------|----------|
| `ue_play` | Start PIE | Observe IK correction, physical animation, or ragdoll behavior live |
| `ue_get_logs` | Stream physics and animation warnings | `category="LogAnimation"`, `minVerbosity="Warning"` — catch bone-chain errors and IK convergence warnings |
| `ue_execute_python` | Query physics simulation state at runtime | `skeletal_mesh_component.is_simulating_physics()`, `get_bone_transform(bone_name)` |
| `take_screenshot` | Capture IK or ragdoll pose | Visual regression: confirm foot placement, IK reach limit, or ragdoll death pose |
| `search_assets` | Find Physics Asset or Control Rig asset | Locate assets for CDO inspection or `get_asset_properties` |
