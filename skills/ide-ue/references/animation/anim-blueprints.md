...
|------|---------|----------|
| `ue_play` | Start / stop PIE | Trigger the character movement / state that drives the AnimBP |
| `ue_get_logs` | Stream animation diagnostic output | `category="LogAnimation"`, `minVerbosity="Warning"` — catch thread-safety violations and missing bone errors |
| `ue_execute_python` | Query AnimBP variable values at runtime | Read blend weights, state machine state, or root motion flags via `skeletal_mesh_component.get_anim_instance()` |
| `search_assets` | Find AnimBP and skeleton assets | Locate assets for `get_asset_properties` review |
| `get_asset_properties` | Read AnimBP CDO defaults | Inspect skeleton assignment, root motion mode, retargeting source |
| `take_screenshot` | Capture a pose mid-animation | Visual check of a transition, IK pose, or root motion displacement |
