---
name: ue-code-authoring-claude-v10
...
## Verifier-aware acceptance

If the task exposes verifier or test sources, read only the checks that cover the requested C++
surface before editing. Treat their required declaration shape as part of the acceptance contract.

### Deterministic codegen gate

Before editing, turn every relevant verifier predicate into a literal checklist: required class
metadata, public declaration and property spelling, source-file state transitions, idempotency guard,
and null/boundary guard. Do not replace a requested mechanism with an equivalent-looking abstraction
unless the check accepts it. A green C++ build and Blueprint compile do **not** compensate for a missed
checklist item.

Before the first build, audit the changed header and source against this checklist. For each item,
point to the exact declaration or branch that satisfies it; otherwise fix it first. In particular:

- keep all requested reflected metadata and access specifiers together on the declaration the
  contract names;
- make each requested state transition explicit in the `.cpp`, including the inactive/no-op path;
- place the duplicate/valid-handle early return immediately before resource or effect creation;
- use direct guards at every boundary where a required dependency is read or used;
- keep required lifecycle checks, resource ownership, and cleanup visible in the implementation.

Only after this source-level contract audit may a build be treated as validation evidence.

- Put every requested virtual override and public API declaration in the header, even when its
  implementation belongs in the `.cpp` file.
- Keep required class metadata visibly on the requested declaration.
- For state transitions, write direct idempotency guards before applying a resource and explicit
  cleanup branches when deactivating it.
- Write direct null guards for every requested boundary dependency (owner, component, ability
  system, configured effect) rather than hiding them in a helper the verifier cannot inspect.

Make a short checklist from those checks before writing code. Before starting a build, re-read the
changed declarations and implementation and verify each item appears in the required location.

- Keep every requested public enum's visibility, underlying type, and enumerator names exact.
- Expose requested Blueprint-readable runtime state as a reflected `BlueprintReadOnly` property,
  not only through a getter.
- Put an explicit early-return guard before applying a resource when repeated activation must be
  idempotent.
- Express each requested isolation dependency as a direct null guard in the relevant path.
...
3. `build_solution_start`; poll `build_solution_state`; fix build errors in changed files.
...
- **Instantiability and costs** — a gameplay ability created for a task must be instantiable; do not mark it abstract unless the request explicitly makes it a base class. Apply a resource cost exactly once through the ability cost path as an additive negative modification to the resource attribute. Do not calculate and overwrite a new balance, and do not gate a locally predicted spend behind an authority-only check. A balance exactly equal to the requested cost must activate and reach zero; a failed activation must neither spend the resource nor add a cooldown.
