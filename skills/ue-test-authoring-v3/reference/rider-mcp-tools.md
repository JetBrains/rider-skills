...
All tools are invoked through `execute_tool(command="<tool> --flag value ...")` in `ue-test-authoring-v3`. Do not call underlying `mcp__<prefix>__<tool>` handles directly.
...
execute_tool(command="lint_files --files '[\"Source/LyraGameTests/Private/FooTests.cpp\"]'")
```
...
Required flag: `--q <literal>`. Optional list-shaped flags such as `--paths` must be JSON arrays, for example `--paths '["Source/LyraGame/Tests/**"]'`.

### `search_file`
Find files by glob pattern.
Required flag: `--q <glob>`. Optional list-shaped flags such as `--paths` must be JSON arrays.
...
Required flag: `--files '["<path>"]'`; `files` is always a JSON array.
...
### `reformat_file`
Reformat files using IDE code style. Required flag: `--files '["<path>"]'`; `files` is always a JSON array.
