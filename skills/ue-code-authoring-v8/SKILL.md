...
| Rename or move an existing symbol safely | `rename_refactoring`, `move_type_to_namespace`, or `change_api_signature` with `--preview true` first |
...
- Rider semantic edit tools are for refactors of existing code: rename, safe delete, extract, move type, change signature, and reorganize namespaces. Use them when the requested edit exactly matches the tool. For freeform new C++ implementation, use normal Codex file edits, then Rider diagnostics/format/build.
...
1. Locate existing patterns and module dependencies.
   - When wiring callbacks, delegates, events, virtual overrides, or message/listener APIs, inspect the exact declaration of the member being used and one real bind/broadcast/remove site for that same member type. Do not copy a nearby binding style unless it is the same declared type.
2. Use Rider semantic refactoring tools when the edit is a supported refactor of existing code. Otherwise edit source with Codex file tools.
...
3. Start a Rider build only after changed-file diagnostics are clean. Do not start or keep polling a build while `get_file_problems` or `lint_files` still reports errors in changed files. Do not dismiss changed-file include, reflection, or generated-code diagnostics as indexing artifacts unless a re-run after `reformat_file`/file refresh is clean or a build has already compiled the same file successfully.
4. `build_solution_start`; poll `build_solution_state` until complete; fix build errors in changed files.
5. `get_project_problems` after a successful build, filtered to changed files.
6. `reformat_file --files '["<path1>","<path2>"]'` on changed files.
...
- Public/requested surface: when the prompt says `public API`, tests inspect it, or designers inspect defaults/state, place the named declarations in the C++ `public:` section. Do not move them to `protected:`/`private:` or hide them behind differently named backing fields.
- Reflected API: requested `UCLASS`, `USTRUCT`, `UENUM`, `UPROPERTY`, and `UFUNCTION` metadata is present on the declaration that callers or designers will use. Preserve standard UE metadata spelling and generated-header include order. If designers must read a state/value from Blueprint, expose the named state/value itself with `BlueprintReadOnly`, not only a C++ getter.
- Defaults: requested default values are visible where tests, designers, or default objects can inspect them. Prefer an in-class initializer for simple reflected defaults, and verify the declaration still lives in the requested access section.
- Blueprint/editor access: use `BlueprintReadOnly` for readable state/config unless mutation is requested; use `EditAnywhere` or the requested edit scope for designer-configurable values. Use `BlueprintAssignable` for requested Blueprint-bindable events.
...
- Callback contracts: for every delegate/event/listener bind, verify the delegate macro/type, supported bind/remove methods, handler signature, and broadcast parameter order from source before writing the handler. Use `AddDynamic` only for dynamic multicast delegates with `UFUNCTION` handlers, `AddUObject` only for native multicast delegates that support it, and match cleanup to the exact bind method or handle returned.
...
When adding Gameplay Ability System code, inspect the nearest existing attribute set, gameplay ability, native tag registration, and cost/cooldown pattern before writing new code. Match the project's exact macros and lifecycle hooks.

- Attribute sets: expose requested attributes with the project's accessor macros and Blueprint-readable getters/events. Clamp both in value-change paths and when the maximum changes so callers never observe values outside the requested range.
- Gameplay abilities: implement activation failure, cost, cooldown, montage playback, and tag grants using the project's existing ability APIs. A named cost/cooldown must be encoded in ability logic, not only mentioned in comments or defaults.
- Gameplay tags: register new native tags in the same source location and style as neighboring native tags; use the exact requested tag string.
- Effects and handles: track handles created by this code, prevent duplicate application on repeated activation/transition, and remove only those handles in teardown/deactivation.
