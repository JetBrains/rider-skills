...
1. Use `execute_tool(command="search_symbol --q <ClassName>")` to find where the class is declared
2. Read the header with Codex file tools to understand the public interface
3. Use `execute_tool(command="get_symbol_info --filePath <path> --line <line> --column <column>")` to check if a method can return null or has preconditions
4. Use `execute_tool(command="analyze_calls --filePath <path> --line <line> --column <column>")` to trace what the method calls and what state it needs
