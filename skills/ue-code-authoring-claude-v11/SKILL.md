---
name: ue-code-authoring-claude-v11
...
**Never call `sleep` between build polls.** `build_solution_state` is documented safe to poll
repeatedly and returns problems incrementally while `state` is `Running`, so a bare re-poll is both
free and more informative than waiting. A `Bash` `sleep` buys nothing, blocks the turn, and a long one
can outlive the step budget. Re-issue `build_solution_state` directly — the `sessionId` is optional,
so an argument-free call reads the most recent build — and stop when `state` is no longer `Running`.
Ignore `-Werror` problems in files you did not change; a `buildIsSuccess: false` caused only by
pre-existing unrelated problems is a blocker to report, not to fix.
